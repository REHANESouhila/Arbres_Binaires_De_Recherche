public class ArbreBinaire {
    ...
    //Méthodes
    //Ajouter
    public boolean ajouter(Element elem) {
        return ajouter(racine, elem);
    }
    
    private boolean ajouter(Noeud n, Element elem) {
        ...
        if (elem.getCle() < n.getElem().getCle()) {
            ...
            if (n.getGauche() == null) {
                ...
                return ajouter(n.getGauche(), elem);
            }
        }
        // Si l'élément à insérer est plus grand que le nœud courant, insérer dans le sous-arbre droit
            ...
            if (n.getDroite() == null) {
                ...
                // Sinon, continuer l'insertion récursive
                return ajouter(n.getDroite(), elem);
            }
        }
    ...
    //Cherche
    public Element chercher(int cle) {
        return chercher(racine, cle);
    }

    private Element chercher(Noeud noeud, int cle) {
        if (noeud != null && noeud.getElem() != null) {
            //Si la clé de l'élément cherché est égale à la clé du noeud courant
            if (noeud.getElem().getCle() == cle) {
                return noeud.getElem();
            //Si la clé de l'élément cherché est plus grande que le neoud courant, chercher dans le sous-arbre droit
            } else if (noeud.getElem().getCle() < cle) {
                return chercher(noeud.getDroite(), cle);
            //Si la clé de l'élément cherché est plus petite que le noeud courant, chercher dans le sous-arbre gauche
            } else if (noeud.getElem().getCle() > cle) {
                return chercher(noeud.getGauche(), cle);
            }
        }
